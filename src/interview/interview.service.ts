...
export class InterviewService {
  ...
  async getStandardAnswer(
    ...
    if (!session) throw new NotFoundException('会话不存在');
    if (!session.isCompleted) throw new NotFoundException('面试尚未结束，无法查看标准答案');
    ...
    if (!question) throw new NotFoundException(`问题索引 ${questionIndex} 不存在（共 ${assistantMsgs.length} 个问题）`);
